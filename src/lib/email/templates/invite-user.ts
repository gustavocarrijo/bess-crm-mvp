...
export async function getInviteUserTemplate(
  inviteUrl: string,
  host: string,
  inviterName: string,
  locale: string = 'en-US'
) {
  const t = await getEmailTranslator(locale, 'emails.inviteUser')
  return {
          ...
          <tr>
            <td style="padding: 40px; text-align: center;">
              ...
              <p style="color: #666; margin-bottom: 30px;">
                ${t('body', { inviterName, host })}
              </p>
              <a href="${inviteUrl}" style="display: inline-block; background: #346df1; color: white; padding: 14px 28px; text-decoration: none; border-radius: 6px; font-weight: 500;">
                ${t('buttonText')}
              </a>
